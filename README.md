...
An AI-powered journaling app that analyzes your emotions using CBT (Cognitive Behavioral Therapy) techniques. Write a journal entry, get instant structured emotional analysis, and track your emotional trends over time.
...
Edit `config/llm_model.yaml` to set your Ollama model:

```yaml
model_name: "llama3.2"   # or any model you have pulled in Ollama
temperature: 0.0
format: "json"
timeout: 120
num_predict: 2048
...
Starts the API + PostgreSQL together:
...
| `GET` | `/journal/history` | Get past entries (supports `skip` & `limit`) |
| `GET` | `/journal/search` | Semantic vector search for similar entries (`?q=...`) |
...
│   └── 2_Dashboard.py       # Streamlit dashboard & trends
├── ui/
│   └── helpers.py       # Shared CSS, charts, HTML builders
├── config/
